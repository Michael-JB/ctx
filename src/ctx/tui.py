...
def _render_cell(column: StatusColumn, cell: str) -> Text | str:
    """A column cell with its icon substituted and its colour applied."""
    display = status.cell_icon(column, cell)
    style = status.cell_style(column, cell)
...
class CtxTui(App[Request | None]):
    ...
    async def _fetch_cell(self, ctx: Context, index: int) -> None:
        if index == 0:
            ...
            rendered = _render_cell(column, cell) if cell else ""
